...
from keypoints_from_images import *
...
def group_keypoints(all_keypoints_by_type, pafs, pose_entry_size=20, min_paf_score=0.05, demo=False):
    ...
    '''print("pose_entries type:",pose_entries.dtype)
    print("pose_entries:",pose_entries)
    print("pose_entries len :",len(pose_entries))
    print("all_keypoints:",all_keypoints)
    print("all_keypoints len :",len(all_keypoints))'''
    #计算骨骼点之间的向量角
    for i in range(len(pose_entries)):
        #print("all_keypoints[i]:",all_keypoints[i])
        angle_left_hand(all_keypoints)
        angle_left_elbow(all_keypoints)
        angle_left_knee(all_keypoints)
        angle_left_ankle(all_keypoints)
        angle_right_hand(all_keypoints)
        angle_right_elbow(all_keypoints)
        angle_right_knee(all_keypoints)
        angle_right_ankle(all_keypoints)
